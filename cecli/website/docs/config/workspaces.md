...
- **local** — an existing on-disk git root referenced by an absolute `path:`.
...
- Their `root` is **overridden** to point at the project's git root (the in-place `path:` directory for local projects, or the cloned checkout for `repo:` projects), so the agent operates inside that repository.
...
- `/workspace <name> <path>` (ad-hoc, no config file required)
...
- For **local** workspaces, the configured `path:` directories are used in-place — no cloning occurs.
