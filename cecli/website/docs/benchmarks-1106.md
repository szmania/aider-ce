...
[![benchmark results](/assets/benchmarks-1106.svg)](https://cecli.dev/assets/benchmarks-1106.svg)
...
1. On the first try, cecli gives GPT the stub code file to edit and the natural language instructions that describe the problem. This reflects how you code with cecli. You add your source code files to the chat and ask for changes, which are automatically applied.
2. If the test suite fails after the first try, cecli gives GPT the test error output and asks it to fix the code. cecli supports this sort of interaction using a command like `/run pytest` to run and share pytest results in the chat with GPT. You can `/run` whatever tests/linters/etc make sense for your language/framework/situation.
