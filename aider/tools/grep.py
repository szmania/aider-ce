...
class Grep(BaseAiderTool):
    ...
    def run(
        ...
        if not repo:
            self.coder.io.tool_error("Not in a git repository.")
            return "Error: Not in a git repository."

        tool_name, tool_path = self._find_search_tool()
        if not tool_path:
            self.coder.io.tool_error("No search tool (rg, ag, grep) found in PATH.")
            return "Error: No search tool (rg, ag, grep) found."

        try:
            search_dir_path = Path(repo.root) / directory
            if not search_dir_path.is_dir():
                self.coder.io.tool_error(f"Directory not found: {directory}")
            ...
            if exit_status == 0:
                ...
                if len(output_lines) > max_output_lines:
                    ...
                    self.coder.io.tool_warning(f"{tool_name} returned 0 but produced no output.")
            ...
            self.coder.io.tool_error(
                f"Error executing {tool_name} command {cmd_str_info}: {str(e)}"
            )
