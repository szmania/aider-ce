...
class DeleteLine(BaseAiderTool):
    """
        ...
        try:
            ...
            try:
                ...
                if line_num_int < 1 or line_num_int > len(lines):
                    raise ToolError(
                        f"Line number {line_num_int} is out of range (1-{len(lines)})"
                    )
            ...
            if original_content == new_content:
                ...
                return (
                    "Warning: No changes made (deleting line {line_num_int} would not change"
                    " file)"
                )
...
def _execute_delete_line(coder, file_path, line_number, change_id=None, dry_run=False):
    return DeleteLine(coder).run(
        file_path=file_path, line_number=line_number, change_id=change_id, dry_run=dry_run
    )
