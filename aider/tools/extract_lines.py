...
class ExtractLines(BaseAiderTool):
    """
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "source_file_path": {
                            "type": "string",
                            "description": "The path to the file to extract lines from.",
                        },
                        "target_file_path": {
                            "type": "string",
                            "description": (
                                "The path to the file to append extracted lines to (will be"
                                " created if needed)."
                            ),
                        },
                        "start_pattern": {
                            "type": "string",
                            "description": "Pattern marking the start of the block to extract.",
                        },
                        "end_pattern": {
                            "type": "string",
                            "description": "Optional pattern marking the end of the block.",
                        },
                        "line_count": {
                            "type": "integer",
                            "description": (
                                "Optional number of lines to extract (alternative to end_pattern)."
                            ),
                        },
                        "near_context": {
                            "type": "string",
                        ...
                        "dry_run": {
                            "type": "boolean",
                            "description": "If True, simulate the change without modifying files.",
                            "default": False,
                        },
                    },
    ...
    def run(
        ...
        try:
            ...
            abs_source_path = self.coder.abs_root_path(source_file_path)
            rel_source_path = self.coder.get_rel_fname(abs_source_path)

            if not os.path.isfile(abs_source_path):
                self.coder.io.tool_error(f"Source file '{source_file_path}' not found")
                return "Error: Source file not found"

            if abs_source_path not in self.coder.abs_fnames:
                if abs_source_path in self.coder.abs_read_only_fnames:
                    ...
                    self.coder.io.tool_error(f"Source file '{source_file_path}' not in context")
                    return "Error: Source file not in context"

            # --- Validate Target File ---
            abs_target_path = self.coder.abs_root_path(target_file_path)
            rel_target_path = self.coder.get_rel_fname(abs_target_path)
            target_exists = os.path.isfile(abs_target_path)
            target_is_editable = abs_target_path in self.coder.abs_fnames
            target_is_readonly = abs_target_path in self.coder.abs_read_only_fnames
            ...
            # --- Read Source Content ---
            source_content = self.coder.io.read_text(abs_source_path)
            if source_content is None:
                self.coder.io.tool_error(
                    "Could not read source file '{source_file_path}' before ExtractLines"
                    " operation."
                )
            ...
            if end_pattern and line_count:
                self.coder.io.tool_error("Cannot specify both end_pattern and line_count")
            ...
            if not start_pattern_line_indices:
                ...
                self.coder.io.tool_error(err_msg)
            ...
            target_content = ""
            if target_exists:
                target_content = self.coder.io.read_text(abs_target_path)
            ...
            self.coder.io.write_text(abs_source_path, new_source_content)
            self.coder.io.write_text(abs_target_path, new_target_content)
            ...
            try:
                ...
                source_change_id = self.coder.change_tracker.track_change(
                ...
                self.coder.io.tool_error(f"Error tracking source change for ExtractLines: {track_e}")
            ...
            try:
                ...
                target_change_id = self.coder.change_tracker.track_change(
            ...
            self.coder.io.tool_error(f"Error in ExtractLines: {str(e)}\n{traceback.format_exc()}")
