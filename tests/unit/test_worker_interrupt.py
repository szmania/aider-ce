...
def test_worker_interrupt_sets_both_flags_false():
    ...
    with (
        ...
        # Configure AgentService mock to return our target_coder as foreground
        mock_instance = MagicMock()
        mock_instance.foreground_coder = target_coder
...
def test_worker_interrupt_cancels_output_task():
    ...
    target_coder = _make_target_coder()
    mock_output_task = AsyncMock()
    target_coder.io.output_task = mock_output_task
    worker = CoderWorker(target_coder, MagicMock(), MagicMock())
